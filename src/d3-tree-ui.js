...
class TreeUI {
  ...
  onKeydownView(e) {
    const selectedKey = KEY_NUMBER[e.which];
    const isMoveNode = selectedKey ? MOVE_DIRECTION[selectedKey] : null;
    ...
    if (selectedKey === "DELETE") {
      ...
      const direction = selectedKey === "TAB" ? APPEND_DIRECTION.RIGHT : APPEND_DIRECTION.BOTTOM;
      ...
      const isEditingNode = this.nodeList.filter(d => d._isEdit).length > 0;
  ...
  createNodeData(nodeObj) {
    return d3.hierarchy(nodeObj, d => d.children);
  }
  ...
  updateNodesLayout() {
    const isInitialLayout = this.$svg.attr("width") === null;
    ...
    */
    ...
    this.columnCount = d3.max(this.nodeList, d => d.depth) + 1;
    this.rowCount = d3.max(this.nodeList, d => d._verticalIndex) + 1;
  ...
  calculateNodePathD(d) {
    ...
    const margin = MARGIN.NODE.LEFT;
    const currentY = d._y;
    const parentY = d.parent._y;
    const diffY = currentY - parentY;
  ...
  */
  ...
  setLeafLength(node) {
    if (!this.hasChildren(node)) {
      ...
      const showChildren = node.children.filter(d => d._isShow);
  ...
  setNodeNameProperties(node) {
    ...
    const nameSize = new Util().measureTextSize(node.data.name, this.$svg);

    node._nameWidth = nameSize.width;
    node._nameHeight = nameSize.height;

    // 項目名が長い（２行以上表示される）場合に省略表示を行う
    const strEachLine = this.splitStringEachLine(node.data.name);
  ...
  updateBackground() {
    ...
    const $rects = this.$background.selectAll("rect");
    const currentRectCount = $rects.data().length;
    const needRectCount = Math.ceil(this.svgWidth / this.columnWidth);
    ...
    if (currentRectCount < needRectCount) {
      ...
      $rects.each(function(d) {
        ...
        const index = $rect.attr("data-index");
  ...
  appendGradient($defs, gradientId) {
    const $branchGradient = $defs.append("linearGradient")
      ...
      .attr("x2", 0)
      .attr("y2", 1);
    const $fromColor = $branchGradient.append("stop")
      .attr("class", "from")
      .attr("offset", "0%");
    const $toColor = $branchGradient.append("stop")
      .attr("class", "to")
      .attr("offset", "100%");
  }
  showTooltip($nodeName, nodeData) {
    ...
    const tooltipTop = nodeData._y + MARGIN.CONTAINER.TOP - this.rowHeight / 2 - tooltipHeight - tooltipMargin.top;

    $tooltip
      .attr("style", `left:${nodeData._x + tooltipMargin.left}px; top:${tooltipTop}px;`);
  }
  hideTooltip() {
    const $tooltip = this.$svgWrap.selectAll(".node-tooltip").remove();
  }
  updateUserInputNode(node) {
    ...
    this.nodes.each(d => {
      ...
      const inputNodeData = _this.createNodeData({
  ...
  startDragging(target) {
    ...
    const dragNodeData = this.$dragNode.data()[0];
  ...
  appendDragLayer() {
    ...
    const overDragArea = function(d) {
      d3.select(this).classed("is-selected", true);
    };
    const outDragArea = function(d) {
      d3.select(this).classed("is-selected", false);
    };
    const setDragAreaProperties = function($dragArea) {
      $dragArea
        ...
        .classed("is-disabled", function(d) {
          ...
          const layoutIndex = parseInt($target.attr("data-childindex"));
          const isBrothers = d.parent === dragNodeData.parent;
          const isYoungBrother = isBrothers && dragNodeData._childIndex === layoutIndex - 1;
          const isSamePosition = isYoungBrother || d._isDragging;

          // let isDisabled = !isBrothers || isSamePosition || d._isUserInput;
          const isDisabled = isSamePosition || d._isUserInput;

          return isDisabled;
        })
        .attr("width", _this.columnWidth)
        .attr("data-depth", d => d.depth)
        .attr("data-parentid", d => d.parent ? d.parent.data.id : -1)
        .on("mouseover", overDragArea)
        .on("mouseout", outDragArea);
    };
    const appendChildren = function($dragArea, isLast) {
      $dragArea.append("rect")
        .attr("width", _this.columnWidth)
        .attr("height", d => getLayerHeight(d, isLast));
      $dragArea.append("line")
        .attr("x1", 0)
        .attr("y1", d => getLineY(d, isLast))
        .attr("x2", $dragArea.attr("width"))
        .attr("y2", d => getLineY(d, isLast));
    };
    const getLayerHeight = function(d, isLast) {
      let dragAreaHeight = 0;
      const isFirst = d._childIndex === 0;
      const isTop = d._verticalIndex === 0;
      ...
      if (isFirst) {
        ...
        // ノード間をドラッグ可能領域とする
        const brotherNode = _this.getChildren(d.parent)[d._childIndex - 1];

        dragAreaHeight = d._y - brotherNode._y;
      }
      return dragAreaHeight;
    };
    const getLineY = function(d, isLast) {
      const isFirst = d._childIndex === 0;
      let lineY = 0;
    ...
    let $dragArea = this.$nodeWrap.selectAll(".node")
      .append("g")
      .classed("is-first", d => d._childIndex === 0)
      .attr("transform", d => `translate(0, ${-(getLayerHeight(d))})`)
      .attr("data-childindex", d => d._childIndex)
      .call(setDragAreaProperties)
      .call($dragArea => {
        appendChildren($dragArea, false);
      });

    const $lastdragArea = this.$nodeWrap.selectAll(".node--youngest")
      .append("g")
      .classed("is-last", true)
      .attr("transform", "translate(0,0)")
      .attr("data-childindex", d => d._childIndex + 1)
      .call(setDragAreaProperties)
      .call($dragArea => {
  ...
  // ノードのドラッグ移動終了
  endDragging() {
    const $selectedDragArea = d3.select(".tree-dragarea.is-selected");
    const exitFunc = () => {
      if (!this.$dragNode) {
        return;
      }
    ...
    if ($selectedDragArea.data().length > 0) {
      const moveNode = this.$dragNode.data()[0];
      const toParentId = parseInt($selectedDragArea.attr("data-parentid"));
      const toChildIndex = parseInt($selectedDragArea.attr("data-childindex"));
      const toDepth = parseInt($selectedDragArea.attr("data-depth"));
      const isMovedNode = this.isMovedNode(moveNode, toParentId, toChildIndex, toDepth);
  ...
  splitStringEachLine(str) {
    ...
    for (let i = 0, len = str.length; i < len; i++) {
      ...
      const isPossibilityLineBreak = strEachLine[lineCount].length > this.textMinLength;

      if (isPossibilityLineBreak) {

        // 文字幅が１行に表示できる文字を超えている場合は改行
        const strSize = new Util().measureTextSize(strEachLine[lineCount], this.$svg);
  ...
  insertChild(parentNode, insertNode, childIndex) {
    ...
    this.setPropertyForNode(insertNode, "depth", parentNode.depth + 1, val => val + 1);
  }
  ...
  */
  setCommonPropetiesForNode($nodes) {
    $nodes
      .classed("node--oldest", d => d._childIndex === 0)
      .classed("node--youngest", d => d.parent && d._childIndex === this.getChildren(d.parent).length - 1)
      ...
      .classed("node--other", d => {
        ...
        const hasChildren = this.getChildren(d);
        let hasLeaf = false;
      ...
      .classed("node--userinput", d => d._isUserInput)
      .classed("is-close", false);
  }

  // ノードを追加する際に新しいIDを作成する
  createNodeId() {
    let maxId = d3.max(this.nodes.descendants(), d => d.data.id);
  ...
  createNode(dataSet) {
    ...
    const $nodes = this.$nodeWrap.selectAll(".node")
    ...
    const $nodesBg = $nodes.append("rect");
    ...
    const $nodeText = $nodes.append("text")
      ...
      .attr("y", "0.35em")
      .text(d => d._ellipsisName ? d._ellipsisName : d.data.name);

    // 親ノードと子ノードを繋ぐためのパス
    const $nodePath = $nodes.append("path")
    ...
    $nodesBg.attr("height", d => d._nameHeight)
      .attr("class", "node-bg")
  ...
  updateNode(param) {
    ...
    if (param) {
      ...
      case NODE_METHOD.MOVE_NODE:
        ...
        const isSameParent = moveNode.parent && moveNode.parent.data.id === toParentId;
        const isYounger = moveNode._childIndex < toChildIndex;
    ...
    const $newNode = this.createNode(this.nodeList);
    ...
    this.$nodes = this.$nodeWrap.selectAll(".node")
      ...
      .duration(DEFAULT_DURATION)
      .attr("opacity", d => d._isShow ? 1 : 0)
      .attr("transform", d => `translate(${d._x}, ${d._y})`);

    const $delNodes = this.$nodeWrap.selectAll(".node")
      .data(this.nodeList, d => d.data.id)
      .exit()
      .remove();

    const $texts = this.$nodes.selectAll(".node-name")
      .text(d => d._ellipsisName ? d._ellipsisName : d.data.name);
    ...
    if ($newNode !== undefined && $newNode !== null && $newNode.data().length > 0) {
      ...
      const newNodeData = $newNode.data()[0];
  ...
  focusNode(selectNodes) {
    ...
    this.$nodes.each(function(d) {
      ...
      const isSelected = selectIds.indexOf(d.data.id) > -1;
  ...
  moveSelectNode(move_direction) {
    ...
    const selectedNode = selectedNodes[0];
    const { _childIndex, depth } = selectedNode;
    let newSelectNodes;
    let cousinNodes;

    switch (move_direction) {
    case MOVE_DIRECTION.TOP:
      cousinNodes = this.nodeList.filter(d => selectedNode.depth === d.depth && selectedNode._verticalIndex > d._verticalIndex);
    ...
    case MOVE_DIRECTION.RIGHT:
      newSelectNodes = this.nodeList.filter(d => selectedNode === d.parent && d._childIndex === 0 && d.depth === depth + 1);
      break;
    }
  ...
  getSelectedNodes() {
    const selectedNodes = this.$nodeWrap.select(".node.is-selected").data();
  ...
  deleteNode(node) {
    ...
    this.updateNode({
      ...
      confirm: deleteNode => {
        ...
        const hasChildren = this.getChildren(deleteNode) && this.getChildren(deleteNode).length > 0;
        let doDelete = true;
  ...
  deleteNodeData(node, confirmFunction) {
    ...
    // 確認処理を行い、キャンセルした場合は処理を中断する。
    const doConfirm = confirmFunction && typeof confirmFunction === "function";
  ...
  editStartNodeName(node) {
    ...
    const $inputNode = this.$svgWrap.append("input")
      .attr("type", "text")
  ...
  editEndNodeName() {
    const $inputNode = this.$svgWrap.selectAll(".node-textbox");
  ...
  isMovedNode(moveNode, toParentId, toChildIndex, toDepth) {
    const someParent = moveNode.parent && moveNode.parent.data.id === toParentId;
    const someChildIndex = moveNode._childIndex === toChildIndex;
    const someDepth = moveNode.depth === toDepth;
  ...
  appendTempNode(selectedNode, direction) {
    ...
    const tempNodeData = this.createNodeData({
      id: this.createNodeId(),
      name: "",
      children: null,
      _isTemp: true
  ...
  updateToggleChildren() {
    ...
    this.$nodes.each(function(d) {
      const $node = d3.select(this);
      const isParent = _this.hasChildren(d);
      const hasToggle = $node.select(".node-toggle").node() !== null;
      ...
      if (isParent && !hasToggle) {
        ...
        const $circles = $toggle.append("circle")
          .attr("r", circleRadius);

        const $texts = $toggle.append("text")
  ...
  updateLineToChild() {
    ...
    this.$svgWrap.selectAll(".node--other").each(function(d) {
      const $node = d3.select(this);
      const isParent = _this.hasChildren(d);
      const hasLine = $node.select(".node-branch-line").node() !== null;
